(function( $ ) {
  ...
  // It has the name attribute "registration"
	...
	$('.MJB_formSend').bind('submit', function(e){
		...
		var yrLength = dob[2].length;
		console.log(dob+' '+dob[0]+' '+dob[1]+' '+yrLength);

		if ( (dob[0] < 13) || (dob[1] < 13) && (yrLength == 4)){
			if($('form[name="single_lead_form"]').valid()) {
				consolidateFields();
			}
		} else {
			console.log('FAIL');
	...
	function showMessage(data){
            data.action = "lead_form_send_mail";
            return jQuery.post(lead_form_params.ajax_url, data, function(response) {
		 console.log(response);
	});
            
		$.ajax({
            type: "POST",
            url: "../wp-content/plugins/debt_lead_form/mail-send.php",
            data: data,
            success: function(){
            console.log('Done');
            }
	...
	function consolidateFields(){
		...
		serverComm('../wp-content/plugins/debt_lead_form/WiSecureAPI.php', {firstName: firstName, lastName: lastName, number: number, email: email, postcode: postcode, borrowAmount: borrowAmount, depositAmount: depositAmount }, showMessage, 'loadingResults');
	};
